# TBM 1.0 - VST3 & Desktop Integration Guide
...
## 2. VST3 Plugin (C++ Framework + Web UI)

To compile this into a VST3/AU plugin that runs in DAWs, the modern approach is to use a **C++ audio framework** with a Web View wrapper.

### The Architecture:
- **DSP Engine (C++)**: Handles the actual audio processing, MIDI routing, and 808 synthesis.
- **Frontend (React)**: The UI we just built.
- **Bridge**: The framework's `WebBrowserComponent` passes JSON messages between the React UI and the C++ DSP engine.

### Background Features (Hidden Logic)
To make TBM 1.0 feel like a professional hardware sampler, the C++ backend implements several "hidden" features:
...
4. **Swing Engine**: The "Humanize" macro doesn't just add random jitter; it applies a weighted shuffle algorithm inspired by classic hardware timing clocks.
5. **Anti-Aliasing Resampling**: The "Lo-Fi Grit" mode uses a 12-bit decimation algorithm with a specific 26kHz anti-aliasing filter emulation to capture that classic hardware "ring".

### Steps to Build the VST3:
1. Download and install a C++ audio plugin framework (e.g., JUCE, iPlug2).
2. Create a new Audio Plug-In project.
3. Enable the GUI extra module.
...
5. Set up a Javascript bridge to listen to knob changes from React:
   ...
   window.pluginBridge.postMessage(JSON.stringify({ param: 'sub_drive', value: 45 }));
   ```
6. Compile the project to generate your `.vst3` and `.component` files.
