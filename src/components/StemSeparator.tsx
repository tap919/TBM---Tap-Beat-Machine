...
export function StemSeparator() {
  ...
  const [waveformOverrides, setWaveformOverrides] = useState<Record<string, number[]>>({});
  const [playheadPct, setPlayheadPct] = useState<Record<string, number>>({});

  const fileRef = useRef<HTMLInputElement>(null);
  const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const modelMenuRef = useRef<HTMLDivElement>(null);
  const audioRefs = useRef<Record<string, HTMLAudioElement>>({});
  // Keep a reference to each stem's timeupdate handler so it can be removed
  const timeupdateHandlers = useRef<Record<string, () => void>>({});
  ...
  useEffect(() => {
    // Cleanup on unmount: stop all audio and remove timeupdate listeners
    return () => {
      if (pollRef.current) clearInterval(pollRef.current);
      for (const [stemId, a] of Object.entries(audioRefs.current) as [string, HTMLAudioElement][]) {
        const handler = timeupdateHandlers.current[stemId];
        if (handler) a.removeEventListener('timeupdate', handler);
        a.pause();
        a.src = '';
      }
    };
  ...
  const togglePlay = (id: string) => {
    ...
    if (prevId === id) {
      ...
      if (!audioRefs.current[id]) {
        ...
        // Live playhead via timeupdate — store handler so it can be removed
        const timeupdateHandler = () => {
          if (audio.duration && audio.duration > 0) {
            setPlayheadPct(prev => ({ ...prev, [id]: (audio.currentTime / audio.duration) * 100 }));
          }
        };
        timeupdateHandlers.current[id] = timeupdateHandler;
        audio.addEventListener('timeupdate', timeupdateHandler);
        audioRefs.current[id] = audio;

        // Decode for real waveform (fire-and-forget)
        if (!waveformOverrides[id]) {
          fetch(url)
            .then(r => r.arrayBuffer())
            .then(buf => {
              // Use OfflineAudioContext for decoding — doesn't count toward
              // the browser's hardware AudioContext limit (typically 6-8)
              const offlineCtx = new OfflineAudioContext(1, 1, 44100);
              return offlineCtx.decodeAudioData(buf).then(decoded => {
                const ch = decoded.getChannelData(0);
                const bars = 80;
                const blockSize = Math.floor(ch.length / bars);
                ...
                setWaveformOverrides(prev => ({ ...prev, [id]: peaks.map(p => p / max) }));
              });
            })
            .catch(() => { /* fallback to procedural waveform */ });
        }
      } else {
  ...
  const reset = () => {
    if (pollRef.current) clearInterval(pollRef.current);
    pollRef.current = null;
    // Stop all audio and remove listeners
    for (const [stemId, a] of Object.entries(audioRefs.current) as [string, HTMLAudioElement][]) {
      const handler = timeupdateHandlers.current[stemId];
      if (handler) {
        a.removeEventListener('timeupdate', handler);
        delete timeupdateHandlers.current[stemId];
      }
      a.pause();
      a.src = '';
    }
    ...
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0 relative edge-glow-bottom">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] text-neutral-500 font-mono uppercase">Powered by Demucs (open-source)</p>
          </div>
        </div>
        ...
        <div className="relative" ref={modelMenuRef}>
          <button
            ...
            className="flex items-center gap-2 bg-bg-main/70 border border-border-main rounded-lg px-3 py-1.5 text-[13px] font-bold text-neutral-300 hover:border-brand/50 transition-all"
          >
          ...
          {showModelMenu && (
            <div className="absolute right-0 top-full mt-1 w-72 bg-bg-surface border border-border-main rounded-xl shadow-2xl z-50 p-1.5 flex flex-col gap-0.5">
              {MODELS.map(m => (
                ...
                >
                  <span className={`text-sm font-bold ${model === m.id ? 'text-brand' : 'text-neutral-200'}`}>{m.label}</span>
                  <span className="text-xs text-neutral-500 font-mono">{m.desc}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
        ...
        <div className="w-72 shrink-0 flex flex-col gap-4">
          ...
          {status === 'idle' || status === 'error' ? (
            <div
              ...
              className={`flex-1 rounded-xl border-2 border-dashed transition-all flex flex-col items-center justify-center gap-3 cursor-pointer min-h-45 ${
                isDragging
            ...
            >
              ...
              <div className="text-center px-4">
                <p className="text-sm font-bold text-neutral-400 uppercase tracking-widest">Drop Audio File</p>
                <p className="text-xs text-neutral-600 font-mono mt-1">mp3 · wav · flac · aiff · ogg · m4a</p>
              </div>
              <button className="px-4 py-1.5 bg-brand/20 hover:bg-brand/30 text-brand text-[13px] font-bold uppercase rounded-lg border border-brand/40 transition-all">
                Browse Files
              </button>
              <input
                ref={fileRef}
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-bg-main/40 rounded-xl border border-border-main min-h-45">
              {status === 'uploading' || status === 'separating' ? (
                <>
                  <Loader2 size={28} className="text-brand animate-spin" />
                  <div className="text-center w-full px-6">
                    <p className="text-[13px] font-bold text-neutral-300 uppercase tracking-widest mb-2">
                    ...
                    <p className="text-xs font-mono text-neutral-500 mt-1.5">{progress}%</p>
                  </div>
                  <p className="text-xs font-mono text-neutral-600 px-4 text-center truncate w-full">{fileName}</p>
                </>
              ) : status === 'done' ? (
                <>
                  <CheckCircle2 size={28} className="text-emerald-500" />
                  <div className="text-center">
                    <p className="text-[13px] font-bold text-emerald-400 uppercase tracking-widest">Separation Complete</p>
                    <p className="text-xs font-mono text-neutral-600 mt-1 px-4 truncate">{fileName}</p>
                  </div>
                  <button
                    onClick={reset}
                    className="px-3 py-1 text-xs font-bold uppercase text-neutral-500 hover:text-neutral-200 border border-border-main hover:border-neutral-600 rounded-lg transition-all"
                  >
                    New File
                  </button>
                </>
          ...
          {status === 'error' && (
            <div className="flex items-start gap-2 p-3 bg-red-950/50 border border-red-800/40 rounded-xl">
              ...
              <p className="text-[13px] text-red-300 wrap-break-word">{errorMsg}</p>
            </div>
          )}

          {/* Demucs info box */}
          <div className="bg-bg-main/50 rounded-xl border border-border-main p-3 flex flex-col gap-2 shrink-0 vignette">
            <div className="flex items-center gap-1.5">
              <Info size={11} className="text-brand shrink-0" />
              <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Demucs Backend</span>
              {backendInstalled === true && (
                <span className="ml-auto flex items-center gap-1 text-xs font-bold text-emerald-500 uppercase"><span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 dot-glow"></span> Ready</span>
              )}
              {backendInstalled === false && (
                <span className="ml-auto flex items-center gap-1 text-xs font-bold text-red-500 uppercase"><span className="inline-block w-1.5 h-1.5 rounded-full bg-red-500 dot-glow"></span> Offline</span>
              )}
            </div>
        ...
        <div className="flex-1 flex flex-col gap-3 min-h-0 overflow-y-auto custom-scrollbar pr-1">
          {stems.length === 0 && (
            <div className="flex-1 flex items-center justify-center text-neutral-700 text-sm font-mono uppercase tracking-widest">
          ...
          {stems.map(stem => (
            <div
              key={stem.id}
              className={`bg-bg-main/50 rounded-xl border transition-all p-4 flex flex-col gap-3 vignette ${
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-neutral-200 uppercase tracking-wider">{stem.label}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {stem.ready && (
                    <>
                ...
                {stem.ready ? (
                  <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                    <g opacity={stem.muted ? 0.2 : 0.7}>
                      {(waveformOverrides[stem.id] ?? waveformData[stem.id] ?? []).map((h, i) => (
                        <rect
                          key={i}
                          x={`${(i / 80) * 100}%`}
                          y={`${50 - h * 45}%`}
                    ...
                    {playingId === stem.id && (
                      <line
                        x1={`${(playheadPct[stem.id] ?? 0).toFixed(1)}%`}
                        y1="0"
                        x2={`${(playheadPct[stem.id] ?? 0).toFixed(1)}%`}
                        y2="100%"
                        stroke={stem.color}
                        strokeWidth="1.5"
                        opacity="0.8"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-xs font-mono text-neutral-700 uppercase tracking-wider">Pending…</span>
                  </div>
                )}
              </div>
              ...
              {/* Volume slider */}
              <div className="flex items-center gap-2">
                <Volume2 size={9} className="text-neutral-600 shrink-0" />
                <input
                  type="range"
                  min={0}
                  max={100}
                  ...
                  disabled={!stem.ready}
                />
                <span className="text-xs font-mono text-neutral-600 w-6 text-right">{stem.volume}</span>
              </div>
            </div>
          ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                {stems.filter(s => s.ready).map(stem => (
                  <a
                    ...
                    download={`${stem.label.toLowerCase()}.mp3`}
                    className="flex items-center justify-center gap-2 py-2 rounded-xl border border-border-main text-neutral-500 hover:text-neutral-200 hover:border-neutral-600 text-xs font-bold uppercase tracking-wider transition-all"
